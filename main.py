...
import pandas as pd # manipulador dedados tabulados
import datetime # data e hora
import time # tempo somente
...
## Base de Dados: Normalmente é o excel que possui as informações dos pasientes para notificação
...
# Esses mapeamentos são referentes à erros de na base dedados do excel que precisam ser preenchidos, então eles seguem um padrão de chave e valor onde a chave está dentro do excel e o valor é o que queremos que essa chave se transforma, é como se fosse uma função f que recebe um valor e retorna outro valor f(valor no excel) = valor no site
...
# AGRAVO = "A92.0 - FEBRE DE CHIKUNGUNYA" # nome do agravo onde será preeenchido o formulário, esta informaçõa pode mudar para outro agravo como por exemplo "A90 - FEBRE"
agravos_possiveis = ["A90 - DENGUE", "A92.0 - FEBRE DE CHIKUNGUNYA"] # lista de agravos no formulário (no site Sinan), é importante que seja exatamente igual à como está no site.
print("Selecione o agravo desejado:")
for i, agravo in enumerate(agravos_possiveis, 1):
    print(f"[{i:3}] Agravo: {agravo}")
AGRAVO = agravos_possiveis[int(input("\n>> ")) - 1]
...
def obter_base():
    ...
    possiveis_dbfs = [dbf for dbf in listdir("arquivos") if dbf.endswith(".dbf")] # cria uma lista de arquivos de base dedados que terminam com .dbf
    if len(possiveis_dbfs) == 0:
        print("Aviso: não existem bases de dados que terminam com .dbf, por favor, colque as bases de dados, arquivos dentro do local descrito abaixo:")
        print("\t", os.path.join(os.getcwd(), "arquivos"))
        print("Após ter feito isso, rode o programa novamente.")
        exit(1)
    # o trecho de código abaixo é ferente à escolha do arquivo e como ler esse arquivo dependendo de sua extensão (dbf)
    ...
    for i, linha_base in base_df.iterrows():
        ...
        nome_paciente_base = str(linha_base["Nome Paciente"]).lower() # nome do paaciente em letras minúsculas
    ...
    return base_df # retorna a base de dados para ue possa ser usada como variável global
...
def wf(by:str, value:str, try_click:bool=True, return_element:bool=True):
    ...
    while True: # repita infinitamente o seguinte:
        ...
        except: # caso alguma alguma das ações acima dê erro, então tente novamente
...
def preencher_notificacao(linha):
    """Preenche uma notificação com base nos dados presentes dentro da variável linha e verifica se tudo ta certo :D"""
    ...
    wf('xpath', '//*[@id="form:panelNotificacao"]', try_click=False, return_element=False) # encontre o elemento por "xpath" que possua o valor especificado, não tenten clicar nele e nem retorne o elmento.
    ...
    wf('xpath', '//*[@id="form:nuNotificacao"]', try_click=False).send_keys(valor) # encontre o elemento por "xpath" que possua o valor especificado, não tenten clicar nele e retorne o elemento para ser usado como variável para send_keys enviar o valor
    ...
    # '0019402' -> se isso daqui não for um número, então pergunte qual é o número desse ngc
    try:
        int(valor)
    except:
        valor = UNIDADE_MAP[" ".join(linha['Unidade Saúde (ou outra fonte notificadora)'].split()[1:])]
    ...
    old_valor = wf('xpath', '//*[@id="form:notificacao_nomePaciente"]').get_attribute('value') # obtém oq tiver de valor dentro da tag "input"
    ...
    if valor == 'Feminino':
        ...
        for option in options:
            ...
            if option.text.endswith("Não"):
                option.click()
                break
    log['Paciente Gestante'] = "Não" # foi ignorado por pedido da Ana.
    ...
    while True:
        ...
        try:
            ...
            log['Status'] += "Apareceu popUP especial flando que a notificação foi cadastrada e simplesmente mostrava um botão para fazer nova notificação.\n"
            break
    ...
    time.sleep(1)
    opcoes = navegador.find_elements("xpath", '//*[@id="form:tipoUf"]/option') # obtém as opções de UF - Notificação ou Residência (sla oq é isso)
    for opcao in opcoes:
...
def preencher_notificacoes():
    ...
    log_colunas.extend(["Zona", "Data/Hora de Adição", "Status"]) # extende a lista colocando mais algumas colunas que não estaõ dentro de base_df mas que são nescessárias.
    log_df = pd.DataFrame(columns=log_colunas) # define um dataframe vazio somente definido suas colunas :D
    login() # chama a função que faz login no Sinan e libera o webdriver (navegador)
    count = 0
    for i, linha in base_df.iterrows(): # para cada linha da base de dados que já está filtrada, faça oq tiver abaixo:
        print(f"Iniciando preenchimento de formulário {count} de {len(base_df)}...")
        count += 1
        dict_log = preencher_notificacao(linha) # chama a função que preenche a notificação com os dados contidos dentro de "linha", essa função deve retornar um dicionário de log que vai conter as colunas especiificadas as log_colunas
        log_df.loc[len(log_df)] = dict_log # adiciona o dicionário de log na última linha do dataframe
        log_df.to_excel(log_path, index=False) # salva o dataframe atual no arquivo de log (se ele não existe, é criado, caso ele já exista, sobreescreve atualizando :D)
